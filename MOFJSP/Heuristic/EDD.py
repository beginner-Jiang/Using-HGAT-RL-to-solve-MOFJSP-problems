"""
Heuristic rule implementation: Earliest Due Date - EDD
"""
import numpy as np
...
class Job:
    ...
    def get_urgency(self, current_time: float) -> float:
        ...
        # Remaining time = Due date - Current time - Remaining shortest processing time
...
# EDD scheduling algorithm implementation

class EDDScheduler:
    """Scheduler based on EDD (Earliest Due Date) rule"""
    ...
    def schedule_step(self) -> bool:
        ...
        # 1. EDD rule: Select the current operation of the job with the earliest due date
        # If multiple jobs have the same due date, select the job with less remaining processing time
        selected_op, selected_job = min(available_ops_with_jobs,
                                        key=lambda x: (x[1].due_date, x[1].get_remaining_processing_time()))

        # 2. Select machine for the operation (choose the machine with the shortest processing time)
        best_machine_id = selected_op.get_best_machine()
        best_machine = self.machines[best_machine_id]

        # 3. Calculate start time
        # Start time depends on: 1) Completion time of the previous operation in the job 2) Machine available time
        job_ready_time = 0
        if selected_op.op_id > 0:
            # If not the first operation, wait for the previous operation to complete
        ...
        # 6. Update current time (minimum of earliest available times of all machines)
    ...
    def print_schedule(self):
        ...
        print("=" * 80)
        print("EDD Schedule Result (Earliest Due Date First)")
        print("=" * 80)
    ...
    for job_idx in range(num_jobs):
        ...
        for op_idx in range(num_ops):
            ...
            for _ in range(k):
                ...
                # Add only if machine ID is within valid range
...
def visualize_gantt_chart(jobs: List[Job], machines: List[Machine],
                          title="EDD Rule - Gantt Chart", save_path=None):
...
def main():
    """Main function: reads file, executes EDD scheduling, displays and saves Gantt chart, exports metrics to Excel"""
    print("=" * 80)
    print("Flexible Job Shop Scheduling based on EDD (Earliest Due Date) Rule (File-driven)")
    print("=" * 80)
    ...
    print("\nJob Due Dates and Minimum Possible Completion Times:")
    for job in jobs:
    ...
    # 2. Execute EDD scheduling
    scheduler = EDDScheduler(jobs, machines)
    scheduler.run_schedule()

    # 3. Print scheduling results and obtain metrics
    edd_metrics = scheduler.print_schedule()

    # 4. Draw and save Gantt chart
    print("\nGenerating Gantt Chart...")
    visualize_gantt_chart(jobs, machines, save_path="../Figure_And_File/Heuristic/EDD/gantt_chart_edd.png")
    ...
    try:
        ...
        summary_data = {
            ...
            'Value': [
                edd_metrics['makespan'],
                edd_metrics['load_balance'],
                edd_metrics['total_tardiness'],
                edd_metrics['avg_tardiness'],
                edd_metrics['tardy_ratio']
            ]
        }
        df_summary = pd.DataFrame(summary_data)

        # Create tardy job details DataFrame
        tardy_jobs_list = []
        for job_id, comp_time, due, tard in edd_metrics['tardy_jobs']:
        ...
        excel_file = '../Figure_And_File/Heuristic/EDD/EDD_metrics.xlsx'
