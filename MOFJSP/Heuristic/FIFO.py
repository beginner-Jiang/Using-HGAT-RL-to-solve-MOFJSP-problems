"""
Heuristic rule implementation: First-Come, First-Served - FIFO
"""
import numpy as np
...
class Job:
    ...
    def __init__(self, job_id: int, operations: List[Operation], due_date: float, release_time: float = 0):
        ...
        self.due_date = due_date          # Due date (for performance evaluation)
        self.release_time = release_time  # Job arrival time (basis for FIFO, default is 0)
        self.current_op_index = 0         # Index of the current operation to be scheduled
        self.remaining_ops = len(operations)  # Number of remaining operations
        self.remaining_processing_time = sum(min(op.machine_times.values()) for op in operations)  # Total remaining shortest processing time
    ...
    def get_remaining_processing_time(self) -> float:
        """Returns the total remaining shortest processing time"""
        return self.remaining_processing_time

    def get_urgency(self, current_time: float) -> float:
        """Calculates job urgency (not used in FIFO)"""
...
class Machine:
    ...
    def assign_operation(self, operation: Operation, start_time: float) -> float:
        """Assigns an operation to the machine, returns the completion time (only allows assignment to the optimal machine)"""
        best_machine_id = operation.get_best_machine()
        if best_machine_id != self.machine_id:
            return float('inf')  # Not the optimal machine, do not assign
...
class FIFOScheduler:
    """Scheduler based on FIFO (First In, First Out, i.e., earliest arrival first) rule"""
    ...
    def get_available_operations(self) -> List[Tuple[Operation, Job]]:
        """Gets all schedulable operations (current unscheduled operation of each job)"""
        available_ops = []
    ...
    def get_current_makespan(self) -> float:
        """Gets the current maximum completion time"""
        makespan = 0
    ...
    def schedule_step(self) -> bool:
        ...
        # FIFO rule: Select the current operation of the job with the earliest arrival time
        # If arrival times are the same, select the one with smaller job ID (entered system first)
        selected_op, selected_job = min(available_ops_with_jobs,
                                        key=lambda x: (x[1].release_time, x[1].job_id))

        # Select the optimal machine for the operation
        best_machine_id = selected_op.get_best_machine()
        best_machine = self.machines[best_machine_id]

        # Calculate the job's allowed start time (completion time of the preceding operation)
        job_ready_time = 0
        ...
        # Update current time (minimum of all machines' earliest available time)
    ...
    def calculate_metrics(self) -> Dict[str, float]:
        ...
        # 2. Machine load balance (Coefficient of Variation)
    ...
    def print_schedule(self):
        ...
        print("=" * 80)
        print("FIFO Schedule Result (First Come First Served, Earliest Arrival First)")
        print("=" * 80)
        ...
        for machine in self.machines:
            ...
            for start, end, op in machine.schedule:
                ...
                print(f"  Job{op.job_id}-Operation{op.op_id}: [{start:.1f} - {end:.1f}], Job Arrival Time: {job_release}")
        ...
        print(f"  Machine Load Balance (CV): {metrics['load_balance']:.4f}")
        ...
        for job in self.jobs:
            ...
            print(f"  Job{job.job_id}: Arrival Time={job.release_time}, Due Date={job.due_date}, "
                  f"Completion Time={completion_time:.1f}, Tardiness={tardiness:.1f}, Status={status}")
...
def read_fjsp_instance(file_path: str) -> Tuple[List[Job], List[Machine]]:
    ...
    Note: Standard instances do not contain job arrival times. This function sets all job arrival times to 0 by default.
          To reflect the FIFO rule, you can modify here or specify externally.
    ...
    for job_idx in range(num_jobs):
        ...
        # Default arrival time for all jobs is 0. Modify here if you need to simulate different arrival times.
        release_time = 0.0
...
def main():
    """Main function: reads file, executes FIFO scheduling, displays and saves Gantt chart, exports metrics to Excel"""
    print("=" * 80)
    print("Flexible Job Shop Scheduling based on FIFO (First Come First Served) Rule (File-driven)")
    print("=" * 80)
    ...
    print("\nJob Arrival Time and Due Date:")
    for job in jobs:
        min_total_time = sum(min(op.machine_times.values()) for op in job.operations)
        print(f"  Job{job.job_id}: Arrival Time={job.release_time:.1f}, "
              f"Minimum Completion Time={min_total_time:.1f}, Due Date={job.due_date:.1f}")

    # 2. Execute FIFO scheduling
    ...
    # 6. Schedule feasibility verification
    print("\n" + "=" * 80)
