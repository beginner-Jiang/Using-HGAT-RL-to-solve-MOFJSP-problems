...
# MWKR调度算法实现
# MWKR规则：优先选择剩余最短加工时间总和最小的作业的当前工序

class MWScheduler:
    """基于MW（最小剩余工作量优先）规则的调度器"""
    ...
    def schedule_step(self) -> bool:
        ...
        # MW规则：选择剩余最短加工时间总和最小的作业的当前工序
        # 若剩余加工时间相同，则选择交货期更早的作业（打破平局）
...
def main():
    """主函数：读取文件，执行MW调度，展示并保存甘特图，导出指标到Excel"""
    print("=" * 80)
    print("基于MW（最小剩余工作量优先）规则的柔性作业车间调度（文件驱动）")
    print("=" * 80)
    ...
    for job in jobs:
        ...
        print(f"  作业{job.job_id}: 最短加工总时间={min_total_time:.1f}, 交货期={job.due_date:.1f}, "
