"""
Heuristic rule implementation: Most Work Remaining - MWKR
"""
import numpy as np
...
class Job:
    ...
    def complete_current_operation(self):
        ...
        if self.current_op_index < len(self.operations):
            # Update remaining operation count and remaining processing time
    ...
    def get_urgency(self, current_time: float) -> float:
        """Calculates job urgency (remaining time buffer)"""
...
# MWKR scheduling algorithm implementation
# MWKR rule: prioritize the current operation of the job with the smallest total remaining shortest processing time

class MWScheduler:
    """Scheduler based on MW (Most Work Remaining) rule"""
    ...
    def get_available_operations(self) -> List[Tuple[Operation, Job]]:
        """Gets all schedulable operations and their associated jobs (first unscheduled operation of each job)"""
        available_ops = []
    ...
    def schedule_step(self) -> bool:
        ...
        # MW rule: Select the current operation of the job with the smallest total remaining shortest processing time
        # If remaining processing times are equal, select the job with the earliest due date (tie-breaker)
        selected_op, selected_job = min(
            available_ops_with_jobs,
            key=lambda x: (x[1].get_remaining_processing_time(), x[1].due_date)
        )

        # Select the machine with the shortest processing time for the operation
        best_machine_id = selected_op.get_best_machine()
        best_machine = self.machines[best_machine_id]

        # Calculate start time: consider the completion time of the previous operation in the job and machine availability
        job_ready_time = 0
        ...
        # Update current time (minimum of earliest available times of all machines)
    ...
    def calculate_metrics(self) -> Dict[str, float]:
        ...
        load_balance = 0 if avg_load == 0 else np.sqrt(np.mean([(load - avg_load) ** 2 for load in machine_loads])) / avg_load
    ...
    def print_schedule(self):
        ...
        print("=" * 80)
        print("MW Schedule Result (Most Work Remaining First)")
        print("=" * 80)
...
def visualize_gantt_chart(jobs: List[Job], machines: List[Machine],
                          title="MW Rule - Gantt Chart", save_path=None):
...
def main():
    """Main function: reads file, executes MW scheduling, displays and saves Gantt chart, exports metrics to Excel"""
    print("=" * 80)
    print("Flexible Job Shop Scheduling based on MW (Most Work Remaining) Rule (File-driven)")
    print("=" * 80)
    ...
    for job in jobs:
        ...
        print(f"  Job{job.job_id}: Minimum Total Processing Time={min_total_time:.1f}, Due Date={job.due_date:.1f}, "
              f"Slack Time={job.due_date - min_total_time:.1f}")

    # 2. Execute MW scheduling
    scheduler = MWScheduler(jobs, machines)
    scheduler.run_schedule()

    # 3. Print scheduling results and obtain metrics
    mw_metrics = scheduler.print_schedule()

    # 4. Draw and save Gantt chart
    print("\nGenerating Gantt Chart...")
    visualize_gantt_chart(jobs, machines, save_path="../Figure_And_File/Heuristic/MWKR/gantt_chart_mw.png")
    ...
    try:
        ...
        summary_data = {
            ...
            'Value': [
                mw_metrics['makespan'],
                mw_metrics['load_balance'],
                mw_metrics['total_tardiness'],
                mw_metrics['avg_tardiness'],
                mw_metrics['tardy_ratio']
            ]
        }
        ...
        tardy_jobs_list = []
        for job_id, comp_time, due, tard in mw_metrics['tardy_jobs']:
        ...
        excel_file = '../Figure_And_File/Heuristic/MWKR/MW_metrics.xlsx'
